...
**Maintain volume replica quorum during node upgrades**
 ...
 cStor volume replicas need to be in quorum when MySQL application is deployed as `deployment` and cStor volume is configured to have `3 replicas`. Node reboots may be common during Kubernetes upgrade. Maintain volume replica quorum in such instances. See [here](/1.0.0-RC1/docs/next/k8supgrades.html) for more details.
